...
# This file initiates and runs the trajectory-based simulation for Single-Sided 0-degree Configuration with short spots 
# and the specified operational policies. 
########################################################################################################################################################
...
assert mode == 'short'
assert side == 'single'
A = LOT_LENGTH * (LOT_WIDTH + LANE_WIDTH)
A_s = LOT_LENGTH * (LOT_WIDTH + 8 + LANE_WIDTH)
...
def eval_Ns(N, N_s):
	...
	for k in range(0, 1 + N_s):	
		...
		test = systemShort(N, N_s, k, SEED_SERV[N-1], SEED_POUT[N-1], SEED_PLIN[N-1], SEED_DRIV[N-1])
...
if (not debug):
	...
	writer.writerow( ['N', 'N_s', 'area', 'cutoff', 'thrupt', 'std thrupt'] )
...
for N in range(1, 1 + LOT_TOTAL):
	...
	if (not debug):
		writer.writerow([N, N_s, opt_thrupt[N][N_s][3], opt_thrupt[N][N_s][0], opt_thrupt[N][N_s][1], opt_thrupt[N][N_s][2]])		
