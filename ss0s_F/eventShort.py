...
class systemShort(system):
	...
	def __init__(self, N, N_s, pol, seedSERV = None, seedPOUT = None, seedPLIN = None, seedDRIV = None):
		...
		super(systemShort, self).__init__(N, N_s, pol, seedSERV, seedPOUT, seedPLIN, seedDRIV)
	...
	def enter_system(self, curr_vehicle, debug_idx = None):
		...
		for j in sorted(J_star, reverse = (not free_curb)):
			...
			if (J < car.stop - 1) or (car.status == 6):
				...
				if ((car.stop - 1) * LOT_LENGTH - curr_vehicle.curr_loc) / rateDRIV >= max(0.0, meanDRIV * self.m_in[car.type][car.j-1] - (self.curr - car.plin_start)):
					J_new.append(j)
				...
				if not car.curr_loc >= LOT_LENGTH + curr_vehicle.curr_loc - SMALL_INTERVAL:
					import pdb; pdb.set_trace()
				if (car.curr_loc - LOT_LENGTH - curr_vehicle.curr_loc) / rateDRIV >= meanDRIV * self.m_in[car.type][car.j-1]:
					J_new.append(j)
		...
		if J_new != []:
			...
			if car.status == 2:
				assert car.j != j
				assert car.plin_start <= self.curr
				assert ( ((car.stop - 1) * LOT_LENGTH - curr_vehicle.curr_loc) / rateDRIV < meanDRIV * self.m_in[car.type][car.j-1] - (self.curr - car.plin_start) )
				car_time = meanDRIV * self.m_in[car.type][car.j-1] - (self.curr - car.plin_start) - ((car.stop - 1) * LOT_LENGTH - curr_vehicle.curr_loc) / rateDRIV 
				...
				assert car.status == 1
				assert car.j != j
				assert (car.curr_loc - LOT_LENGTH - curr_vehicle.curr_loc) / rateDRIV < meanDRIV * self.m_in[car.type][car.j-1]
				car_time = meanDRIV * self.m_in[car.type][car.j-1] - (car.curr_loc - LOT_LENGTH - curr_vehicle.curr_loc) / rateDRIV
			...
			for idx in range(len(J_new)-1, -1, -1):
				...
				if (J < car.stop - 1) or (car.status == 6):
					...
					if ((car.stop - 1) * LOT_LENGTH - curr_vehicle.curr_loc) / rateDRIV < max(0.0, meanDRIV * self.m_in[car.type][car.j-1] - (self.curr - car.plin_start)):
						J_new = J_new[:idx]
						if idx == 0:
							car_time = max(car_time, meanDRIV * self.m_in[car.type][car.j-1] - (self.curr - car.plin_start) - ((car.stop - 1) * LOT_LENGTH - curr_vehicle.curr_loc) / rateDRIV)
						break
					...
					if (car.curr_loc - LOT_LENGTH - curr_vehicle.curr_loc) / rateDRIV < meanDRIV * self.m_in[car.type][car.j-1]:
						J_new = J_new[:idx]
						if (idx == 0):
							car_time = max(car_time, meanDRIV * self.m_in[car.type][car.j-1] - (car.curr_loc - LOT_LENGTH - curr_vehicle.curr_loc) / rateDRIV)
						break
	...
	def check_lane_zero_short(self, curr_vehicle, first_attempt, delay_reason = None, delay_status = None, delay_speed = None, curr_time = None):
		...
		while car != None:
			...
			if car.idx == idx:
				...
				if car.stop == stop + 1 and curr_time < car.plin_end - meanDRIV * self.m_out[curr_vehicle.type][curr_vehicle.j-1]:
					delayed = True
				...
				elif (control == 'full' or ptype == 0) and car.stop > stop + 1 and est_pout_end + (car.stop - 1 - stop) * LOT_LENGTH / rateDRIV < max(curr_time, car.plin_start + meanDRIV * self.m_in[car.type][car.j-1]) - SMALL_INTERVAL:
					...
					# assert car.plin_start + meanDRIV * self.m_in[car.type][car.j-1] > est_pout_start + meanDRIV * self.m_out[curr_vehicle.type][curr_vehicle.j-1] + (car.stop - 1 - stop) * LOT_LENGTH / rateDRIV + SMALL_INTERVAL
					if meanDRIV * self.m_in[car.type][car.j-1] + car.plin_start - (meanDRIV * self.m_out[curr_vehicle.type][curr_vehicle.j-1] + (car.stop - 1 - stop) * LOT_LENGTH / rateDRIV) > req_time:
						delayed = True
				...
				# elif car.stop > stop and car.status == 1:
					...
					if car_time < car.end_time + meanDRIV * self.m_in[car.type][car.j-1] - 2e-05:
						# if not car.end_time + meanDRIV * self.m_in[car.type][car.j-1] > est_pout_start + meanDRIV * self.m_out[curr_vehicle.type][curr_vehicle.j-1] + (car.stop - stop - 1) * LOT_LENGTH / rateDRIV:
						# 	import pdb; pdb.set_trace()
						# else:
						delayed = True
						car_time = car.end_time + meanDRIV * self.m_in[car.type][car.j-1] - meanDRIV * self.m_out[curr_vehicle.type][curr_vehicle.j-1] - (car.stop - stop - 1) * LOT_LENGTH / rateDRIV
						req_time = max( req_time, car_time)
				...
				stopped = False
				assert car.stop < stop or (car.stop == stop and car.j != curr_vehicle.j)
				...
				assert (car.status == 6) or (car.status == 1 and car.stop >= stop - 1 and car.j != curr_vehicle.j)
				...
				if stopped and car.prev.status == 1 and car.prev.j == curr_vehicle.j:
					...
					if not (car.prev.stop - 1) * LOT_LENGTH >= car.curr_loc - 1.5e-04:
						import pdb; pdb.set_trace()
					car_time = max(meanDRIV * self.m_in[car.prev.type][car.prev.j-1] + car.prev.end_time, self.curr + max(0.0, ((car.prev.stop - 1) * LOT_LENGTH - car.curr_loc)) / rateDRIV) + (stop - car.prev.stop) * LOT_LENGTH / rateDRIV
				elif stopped:
